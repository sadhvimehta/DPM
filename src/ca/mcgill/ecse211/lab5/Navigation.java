...
public class Navigation {
  ...
  public void turnTo(double theta) {
    ...
    if (Math.abs(travelangle) < Math.PI) {
      ...
      if (travelangle > Math.PI) {
        leftMotor.rotate(
            -convertAngle(
                WHEEL_RADIUS, TRACK, Math.toDegrees(Math.abs(2 * Math.PI - Math.abs(travelangle)))),
            true);
        rightMotor.rotate(
            convertAngle(
                WHEEL_RADIUS, TRACK, Math.toDegrees(Math.abs(2 * Math.PI - Math.abs(travelangle)))),
  ...
  public void turnCW(long degree) {
    leftMotor.rotate(convertAngle(LabFiveMain.WHEEL_RADIUS, LabFiveMain.TRACK, degree), true);
    rightMotor.rotate(-convertAngle(LabFiveMain.WHEEL_RADIUS, LabFiveMain.TRACK, degree), true);
  }
