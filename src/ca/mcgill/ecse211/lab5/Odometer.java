...
import ca.mcgill.ecse211.lab5.LabFiveMain;
		    ...
		    while (true) {
		      ...
		      synchronized (lock) {
		    	double distL, distR, deltaD, deltaT, dX, dY;
		  		...
		  		distL = Math.PI*LabFiveMain.WHEEL_RADIUS*(currentLeftMotorTachoCount-lastLeftMotorTachoCount)/180;		// compute L and R wheel displacements
		  		distR = Math.PI*LabFiveMain.WHEEL_RADIUS*(currentRightMotorTachoCount-lastRightMotorTachoCount)/180;
		  		...
		  		if(theta >= (2*Math.PI)){							// if theta crosses 360, return theta to zero and compute w.r.t zero degrees
		  			...
		  			theta = (2*Math.PI) + theta;					// if theta goes below zero, return theta to 360 and compute w.r.t 360 degrees
		  		}
