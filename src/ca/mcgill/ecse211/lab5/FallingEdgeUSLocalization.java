...
public class FallingEdgeUSLocalization {
  ...
  public void doLocalization() {
    ...
    if (getFilteredData() < DISTANCE_WALL + 10) {
      ...
      this.leftMotor.rotate(
          Navigation.convertAngle(LabFiveMain.WHEEL_RADIUS, LabFiveMain.TRACK, 180), true);
      this.rightMotor.rotate(
          -Navigation.convertAngle(LabFiveMain.WHEEL_RADIUS, LabFiveMain.TRACK, 180), false);
      odo.setTheta(0);
    }
  ...
  // method that turns the rpbot to a specific angle, so it compares between the angle we had and
  // the angle we want to go to
  // inputs dest angle
  public void turnToDestintaionAngle(double angle) {
    ...
    this.leftMotor.rotate(
        convertAngle(LabFiveMain.WHEEL_RADIUS, LabFiveMain.TRACK, Math.toDegrees(delta)), true);
    this.rightMotor.rotate(
        -convertAngle(LabFiveMain.WHEEL_RADIUS, LabFiveMain.TRACK, Math.toDegrees(delta)), false);
    Sound.beep();
  }
